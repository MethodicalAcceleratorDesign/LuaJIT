...
static size_t fs_prep_var(LexState *ls, FuncState *fs, size_t *ofsvar, size_t *ofsdeclname, const char *declname)
{
...
static void parse_body(LexState *ls, ExpDesc *e, int needself,
                       LJMAD_SYNTAX(int islambda,) BCLine line, char *declname);
...
static void parse_body(LexState *ls, ExpDesc *e, int needself LJMAD_SYNTAX(, int islambda), BCLine line, char *declname)
{
...
static void parse_local(LexState *ls)
{
  ...
  if (lex_opt(ls, TK_function)) {  /* Local function declaration. */
    ...
    parse_body(ls, &b, 0 LJMAD_SYNTAX(, 0), ls->linenumber, log_declname(log));
...
static void parse_func(LexState *ls, BCLine line)
{
  ...
  lj_lex_endlog(ls);
  parse_body(ls, &b, needself LJMAD_SYNTAX(, 0), line, log_declname(log));
  fs = ls->fs;
  bcemit_store(fs, &v, &b);
